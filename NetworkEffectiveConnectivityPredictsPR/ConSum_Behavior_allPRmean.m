...
%{'180807_Sub29'}
%{'180725_Sub25'}
%runs with index of 17 and 44 were failed during model fitting
failed = [17,44];
sub_i_all(failed) = [];
...
mid = [2,3,8];
late = [1,6];
roi_sets = {early,mid,late};

brain_meas = squeeze(mean_B_m1_m2_all(:,:,1,:)); % oddball modulated
...
%% Q1: what edges are related to PR in the block
...
[r,p] = corrcoef(brain_mea(select),behavior_select(select));
    ...
    hold on 
    label = [];
    for lp = 1:length(ELBO_all_EEGSTVROI);
    if((ELBO_all_EEGSTVROI(lp)==-Inf)|(imag(ELBO_all_EEGSTVROI)~=0))
    else
    ...
    load('PR_oddmean_sub_successblocks.mat')
...
%         text(brain_mea_sub(lp)*1.01,behavior_sub(lp)*1.01,num2str(lp),'FontWeight','bold','FontSize',15);
    ...
    a = annotation('textbox', [0.15, 0.65, 0.1, 0.1], 'String', str,'LineStyle','none');
    a.FontSize = 18;
    grid on
    set(gca,'FontSize',20)
